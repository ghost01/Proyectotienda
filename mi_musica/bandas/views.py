...
def vista_gustos(request):
	info_enviado = False
	email = ""
	title = ""
	text = ""
	if request.method == "POST":
		formulario = gustos_form(request.POST)
		if formulario.is_valid():
			info_enviado = True
			email = formulario.cleaned_data['correo']
			title = formulario.cleaned_data['titulo']
			text = formulario.cleaned_data['texto']
			time = formulario.cleaned_data['tiempo']
			
	else:
		formulario = gustos_form()
	return render(request,'gustos.html',locals())
